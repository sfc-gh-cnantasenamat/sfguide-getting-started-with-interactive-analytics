...
| Understand - Use cases | Quickstart | — | Rewrote the section around a three-domain taxonomy (AI & Agents, Customer-Facing Data Apps, Operational Analytics) with new examples (low-cost RAG, AI observability, high-concurrency MCPs, embedded analytics, trading & risk, supply chain & inventory), and framed the shared requirements as low latency, high concurrency, fresh data, and low cost per query. Replaced `use-cases.png` with the matching updated diagram. |
